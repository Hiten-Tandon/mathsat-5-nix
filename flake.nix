{
  ...
  outputs = {nixpkgs, flake-utils, ...}: flake-utils.lib.eachSystem ["x86_64-linux" "aarch64-linux" "aarch64-darwin"]  (system: 
    ...
    let
        pname = "mathsat";
        version = "5.6.11";
        url = if stdenv.isLinux && stdenv.isx86_64 then "https://mathsat.fbk.eu/release/${pname}-${version}-linux-x86_64-reentrant.tar.gz" 
              else if stdenv.isLinux && stdenv.isAarch64 then "https://mathsat.fbk.eu/release/${pname}-${version}-linux-aarch64-reentrant.tar.gz" 
              else "https://mathsat.fbk.eu/release/${pname}-${version}-osx.tar.gz";
